...
for file in PREPROCESS_FILES:
    path = hf_hub_download(
        repo_id="pharma/ci-preprocess", filename=file, cache_dir="cache"
    )
...
config = Config(
    cache_path=cache_path, clip_model_path="cache", clip_model_name="ViT-L-14/openai"
)
...
def image_analysis(image, clip_model_name):
    ...
    medium_ranks = {
        medium: sim
        for medium, sim in zip(
            top_mediums, ci.similarities(image_features, top_mediums)
        )
    }
    artist_ranks = {
        artist: sim
        for artist, sim in zip(
            top_artists, ci.similarities(image_features, top_artists)
        )
    }
    movement_ranks = {
        movement: sim
        for movement, sim in zip(
            top_movements, ci.similarities(image_features, top_movements)
        )
    }
    trending_ranks = {
        trending: sim
        for trending, sim in zip(
            top_trendings, ci.similarities(image_features, top_trendings)
        )
    }
    flavor_ranks = {
        flavor: sim
        for flavor, sim in zip(
            top_flavors, ci.similarities(image_features, top_flavors)
        )
    }
...
with gr.Blocks(css=CSS) as block:
    with gr.Column(elem_id="col-container"):
        ...
        with gr.Tab("Prompt"):
            with gr.Row():
                ...
                with gr.Column():
                    input_model = gr.Dropdown(
                        MODELS, value=MODELS[0], label="CLIP Model"
                    )
                    input_mode = gr.Radio(
                        ["best", "fast", "classic", "negative"],
                        value="best",
                        label="Mode",
                    )
            ...
            examples = [
                ["example01.jpg", MODELS[0], "best"],
                ["example02.jpg", MODELS[0], "best"],
            ]
            ex = gr.Examples(
                examples=examples,
                fn=image_to_prompt,
